...
const CS_PING_INTERVAL_MS = 200;
const CS_PING_TIMEOUT_MS = 6000;

async function waitForContentScript(tabId: number): Promise<boolean> {
  const start = Date.now();
  while (Date.now() - start < CS_PING_TIMEOUT_MS) {
    try {
      const resp = (await chrome.tabs.sendMessage(tabId, { kind: 'PING' })) as { pong?: boolean } | undefined;
      if (resp?.pong) return true;
    } catch {
      /* CS not ready yet */
    }
    await sleep(CS_PING_INTERVAL_MS);
  }
  return false;
}
...
export async function handleStartScrape(p: StartScrapePayload): Promise<void> {
  ...
  let captureListener: ((msg: unknown) => void) | null = null;

  const fail = (): void => {
    if (captureListener) {
      try { chrome.runtime.onMessage.removeListener(captureListener); } catch { /* noop */ }
  ...
  try {
    ...
    captureListener = (raw: unknown) => {
      const msg = raw as { kind?: string; batchId?: string; endpoint?: string; payload?: unknown };
      ...
      if (pool.size === before) {
        ...
        if (stallCount >= STALL_THRESHOLD) {
          ...
          if (kwIndex < p.keywords.length) {
            ...
            // 轮询 PING 直到 CS 重新注入完成，再发 BEGIN_INTERCEPT，避免固定 sleep 的竞态。
            const ready = await waitForContentScript(tabId);
            if (!ready) throw new Error('content script PING timeout after NAVIGATE_KEYWORD');
            await chrome.tabs.sendMessage(tabId, { kind: 'BEGIN_INTERCEPT', batchId: p.batchId });
            await sleep(MAIN_INJECT_DELAY_MS);
            stallCount = 0;
          }
    ...
    console.error('[xhs-radar] handleStartScrape failed:', err);
    fail();
  } finally {
    if (captureListener) {
      try { chrome.runtime.onMessage.removeListener(captureListener); } catch { /* noop */ }
