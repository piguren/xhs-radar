...
| 文档 | 模块 | 状态 | 链接 |
|------|------|------|------|
| L1 产品设计大纲（本文档） | 全局 | ✅ v0.3 | `docs/prd/xhs-radar-l1.md` |
| L2 传统 PRD | 全局 | 🔲 待产出 | — |
...
我们要做的工具，要解决三件事。第一，输入**任何赛道**的一组关键词，自动批量发现当下赛道里的爆款。第二，让用户在一个**像 Web 应用一样的浮层界面**里看完爆款列表与单条正文，并由 AI 辅助提取选题方向、写作结构和趋势关键词。第三，把今天发现的爆款收入"收藏库"，让今天的工作能服务明天的写作；每次抓取自动保存为历史批次，可随时回看或重新载入。
...
```mermaid
flowchart TD
    A["在任意网页点击扩展图标"] --> B["注入式浮层弹出（占屏约 70%）"]
    B --> C["配置页：输入关键词组 + 筛选条件 + 目标爆款数"]
    C --> D["点击「开始抓取」"]
    D --> E{"已登录小红书？"}
    E -->|"否"| F["提示登录 → 用户登录后回到浮层继续"]
...
**2. 浮层式 Dashboard 浏览与 AI 辅助。** 浮层占当前网页约 70%，可在任意网页唤起、与宿主页面互不干扰，关闭即回到原网页。**三个页签**：
...
| 模块 | 职责 |
|------|------|
| **浮层 UI 容器** | 注入式浮层（占屏约 70%），可在任意网页唤起，与宿主页面互不干扰 |
| **配置页** | 关键词组、时间窗口、爆款阈值、目标数量、AI 密钥的设置入口 |
...
| **形态：注入式浮层** | 在任意网页唤起，占屏约 70%，与宿主页面互不干扰，关闭即回到原网页 |
| **本质浏览器插件** | 抓数据靠插件能力（绕开反爬），UI 体感像独立 Web 应用 |
| **仅作者自用** | 一期不做账号、权限、协作 |
...
*xhs-radar L1 v0.3 · idea-to-prd-v1 流程产出 · 2026-05-08*
